...
**Template B v3 是本规范的基准，十二个主题现已全部统一。** 详见 §7。
...
当一个主题是**多集系列**（视频合集、连载等，通常 10 集以上）时，不要把所有内容堆进一个 `index.html`，改成「系列目录页 + 逐集子页」：

```
<系列名>/
├── README.md              # 系列说明：立意、集数表、各集要点、来源
├── index.html             # 【目录页】系列简介 + 讲次目录（链到各集）+ 路线图
├── 01-<主题>/             # 每集一个独立页面，各自自包含
│   ├── index.html         # 该集正文
│   └── assets/            # 该集自己的配图（编号从 fig-01 重新开始）
├── 02-<主题>/
│   ├── index.html
│   └── assets/
└── ...
...
1. **子页必须自包含** —— 每集一个 `index.html` + 自己的 `assets/`，配图编号从 `fig-01` 重新开始，§9 的检查脚本对每个子页独立跑一遍。
2. **目录名用「两位序号-主题」** —— 如 `01-开篇/`、`02-规则系统/`，保证按文件名排序即正确顺序。
3. **页脚给三向导航** —— 上一集、下一集、返回系列目录，末行再给「返回知识库首页」。
4. **集内顶部放「系列导航」时间线** —— 用 `.timeline` 列出全部集数，当前集标「你在这里」，已发布的集给链接。
5. **门户仍只放一张系列卡片** —— 指向系列目录页，不随集数增长；集数变化只改目录页与 `README.md` 的集数表。
6. **每集都有 `30 秒总览`、`一页速查`、`术语表`** —— 术语表只收该集的概念，不跨集累积，保证单页可独立阅读。
...
**十二个主题已全部统一到 Template B v3**，不再存在偏差：
...
| `transformer-series/` | ✅ B v3（视频系列单页合集） |
